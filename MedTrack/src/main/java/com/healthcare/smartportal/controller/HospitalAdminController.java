...
public class HospitalAdminController {
    ...
    public ResponseEntity<?> registerHospitalWithAdmin(@RequestBody HospitalRegistrationRequest request) {
        try {
            Hospital hospital = adminService.registerHospitalAndAdmin(request);
            UUID hospitalId = hospital.getId();
            return ResponseEntity.ok("Hospital and admin registered with hospital ID: " + hospitalId
    ...
    public String login(@RequestParam String username, @RequestParam String password) {
        return adminService.login(username, password).isPresent() ? "Login Successful" : "Invalid credentials";
    }
    ...
    public ResponseEntity<String> updateAntivenomInventory(
        ...
        boolean success = adminService.updateAntivenom(hospitalId, animalName, quantity);

        if (success) {
            return ResponseEntity.ok("Antivenom inventory updated.");
        } else {
            return ResponseEntity.badRequest().body("Failed to update inventory. Please check animal name or hospital ID.");
        }
    }
    ...
    public ResponseEntity<String> updateBloodInventory(
        ...
        boolean success = adminService.updateBlood(hospitalId, bloodGroup, quantity);

        if (success) {
            return ResponseEntity.ok("Blood inventory updated.");
        } else {
            return ResponseEntity.badRequest().body("Failed to update blood inventory. Please check hospital ID or blood group.");
        }
    }
}
